...
function dbConnect(){
  /*//DBへの接続準備
  $dsn = 'mysql:dbname=bookrev;host=localhost;charset=utf8';
  $user = 'root';
  $password = 'root';
  $options = array(
...
function getMsgsAndBord($id){
    ...
    try {
        ...
        $dbh = dbConnect();
        // SQL文作成
        $sql = 'SELECT m.id AS m_id, u.username AS u_name,product_id, bord_id, send_date, from_user, msg, b.create_date FROM message AS m RIGHT JOIN bord AS b ON b.id = m.bord_id RIGHT JOIN users AS u ON u.id = from_user WHERE b.product_id  = :id AND m.delete_flg = 0 ORDER BY send_date ASC';
        $data = array(':id' => $id);
        // クエリ実行
        $stmt = queryPost($dbh, $sql, $data);
...
function getMyMsgsAndBord($u_id){
    ...
    try {
        ...
        // まず、掲示板レコード取得
        // SQL文作成
        $sql = 'SELECT * FROM bord AS b WHERE b.user_id = :id AND b.delete_flg = 0';
        $data = array(':id' => $u_id);
        // クエリ実行
        $stmt = queryPost($dbh, $sql, $data);
        $rst = $stmt->fetchAll();
...
function getBordId($p_id){
  debug('自分のbordID情報を取得します。');
  //例外処理
  try {
      // DBへ接続
      $dbh = dbConnect();
      ...
      // まず、掲示板レコード取得
      // SQL文作成
      $sql = 'SELECT * FROM bord AS b WHERE b.product_id = :id AND b.delete_flg = 0';
      $data = array(':id' => $p_id);
...
function pagination( $currentPageNum, $totalPageNum, $sort, $category, $link = '', $pageColNum = 5){
  ...
  echo '<div class="pagination">';
    echo '<ul class="pagination-list">';
      if($currentPageNum != 1){
        echo '<li class="list-item"><a href="?c_id='.$category.'&sort='.$sort.'&p=1'.$link.'">&lt;</a></li>';
      }
      for($i = $minPageNum; $i <= $maxPageNum; $i++){
        echo '<li class="list-item ';
        if($currentPageNum == $i ){ echo 'active'; }
        echo '"><a href="?c_id='.$category.'&sort='.$sort.'&p='.$i.$link.'">'.$i.'</a></li>';
      }
      if($currentPageNum != $maxPageNum && $maxPageNum > 1){
        echo '<li class="list-item"><a href="?c_id='.$category.'&sort='.$sort.'&p='.$maxPageNum.$link.'">&gt;</a></li>';
      }
    echo '</ul>';
  echo '</div>';
}
